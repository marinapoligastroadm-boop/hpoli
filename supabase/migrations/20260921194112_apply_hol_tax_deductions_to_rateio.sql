  ...
  if new.status in ('received', 'partial')
    ...
    if should_sync then
      ...
      select
        ...
        round(rateio_base * partner.share_percent / 100.0, 2)
...
after update of status, received_amount, rateio_competence, billing_unit
on public.invoices
for each row execute function public.generate_invoice_distribution();

update public.distributions as distribution
set base_amount = round(invoice.received_amount * 0.8807, 2),
    distributed_amount = round(
      round(invoice.received_amount * 0.8807, 2)
        * distribution.share_percent / 100.0,
      2
    ),
    generated_at = now()
from public.invoices as invoice
where invoice.id = distribution.invoice_id
